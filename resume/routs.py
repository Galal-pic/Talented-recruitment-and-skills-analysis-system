...
from resume.utils import extract_skills_from_pdf,sentence_embedding4_similarity_score_test
...
def history():
    return render_template('history.html',title='job description')
...
def cvs():
    if request.method=='POST' and current_user.is_authenticated:
        ...
        for cv in cvs:
            ...
            all_cvs.append((cv[0],seeker.skills,seeker.phone,seeker.email,seeker.country,seeker.linkedin,cv[1]))

        aready_job_description = job_description.query.filter_by(recruiter_email=current_recruiter.email, job_title=job_title ,Job_description = description).first()          
        if current_recruiter and aready_job_description:
                return render_template('history.html', data=all_cvs)
        elif current_recruiter:
            desc = job_description(job_title=job_title,Job_description=description,cvs=str(cvs),
                                    recruiter_email=current_user.email,company = current_recruiter.company,
                                    industry = current_recruiter.industry,username=current_recruiter.username)
...
def seekerregister():
    ...
    if form.validate_on_submit():
        ...
        jobs,skills_list,embedding = append_cvs(filename,'cvs/'+filename,name='seekerregister')
        new_seeker = seekr(
                        fname = form.fname.data,lname=form.lname.data,
                        username = form.username.data,email=form.email.data,
                        linkedin=form.linkedin.data , country=form.country.data,
                        phone = form.phone.data , seniority_level = form.seniority_level.data,
                        current_position = form.current_position.data , password = hashed_password,
                        cv_name = filename,skills = str(skills_list),jobs=str(jobs),embedding=str(embedding))
...
def recruiterregister():
    ...
    if form.validate_on_submit():
        ...
        new_recruiter = recruiter(
                        fname = form.fname.data,lname=form.lname.data,
                        username = form.username.data,email=form.email.data,
                        linkedin=form.linkedin.data , company=form.company.data,
                        industry = form.industry.data ,password = hashed_password)
    ...
    return render_template('recruiterregister.html',title='recruiterregister',form=form)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if new_seeker and bcrypt.check_password_hash(new_seeker.password,form.password.data):
            login_user(new_seeker,remember=form.remember)
            # flash('You have been logged in','success')
            return redirect(url_for('uploadcv'))
        elif new_recruiter and bcrypt.check_password_hash(new_recruiter.password,form.password.data):
...
def pdf():
    if request.method=='POST':
        if 'pdfFile' not in request.files:
            ...
            jobs,_,_ = append_cvs(input_pdf.filename,'cvs/'+input_pdf.filename,name='pdf')
...
def jobfeed():
    ...
    for job in job_feed:
        ...
        job.Job_description = Markup(job.Job_description)  # Mark as safe for HTML rendering
...
def applyjob():
    if current_user.is_authenticated:
        ...
        if seeker:
            ...
            if apply_to_job(company_name, job_title, recruiter_email, seeker.email, seeker.fname, seeker.phone, seeker.linkedin, seeker.skills):
